...
- Original Claude command: `/lisa:queue-status`
- OpenCode invocation: `$lisa-queue-status` or a plain-English request that matches this skill.
- Treat the user's surrounding request as the command arguments.

Use the /lisa:queue-status skill to inspect the current project's configured PRD source and build tracker, report queue-health verdicts, and highlight actionable backlog or stuck-state signals. Use the user's surrounding request as this command's arguments.
